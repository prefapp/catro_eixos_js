...
class Proceso{
    ...
    constructor(tarea){
        ...
        //seteamos un nombre de proceso si no lo hay
        if(!tarea.args.__nombre_proceso__){
            tarea.args.__nombre_proceso__ = "";
        }
    }
    ...
    ejecutar() {
        ...
        if(this.arg("__depurar__")){
            this.__activarDebug(this.arg("__depurar__"));
        }
            ...
            pasos.reduce((promesa, paso) => {
                ...
                return promesa
                    ...
                    .catch((err) => {
                        ...
                        return this.terminar(() =>{ fallo(this.tarea) }, true, err );
    ...
    terminar(cumplida, hayError = false, err){
        ...
        if(hayError){
            if(!this.tarea.resultados.estado){
        ...
        if(this.arg("__subproceso__")){
            
            this.resultado(

                "__estado__",

                this.ejecutor.trazado.getTrazadoProceso()
            );
        }
    ...
    error(err){
        ...
        if(typeof err === "string"){
            err = `[${this.arg("__nombre_proceso__")}][${err}]`
        }     
    ...
    subProceso(nombre, args = {}){
        ...
        this.ejecutor.trazado.ejecucionSubproceso(nombre);
        ...
        return new Promise((cumplida, falla) => {
            ...
            this.refProcesador.ejecutar(
                ...
                this.__comunicarFinSubproceso(tarea);
    ...
    __comunicarFinSubproceso(tarea){

        let estado = tarea.resultados.__estado__;

        this.ejecutor.trazado.ejecutadoSubproceso(estado);

        delete tarea.resultados.__estado__;
    ...
    __ejecutarEvento(evento){

        if(!this.fEventos[evento]) return;

        this.fEventos[evento].forEach((e) => e() )
    }
