...
class DepuradorProceso{
 ...
 //       console.log(e["ultimaAccion"].get("accion")); return;

        let ultimaAccion = estado["ultimaAccion"].get("accion");
        let e = estado["proceso"];
        let cmd = estado["comando"];
        ...
        switch(ultimaAccion){
            ...
            case "EJECUTANDO_COMANDO":
                this.__ejecutandoComando(cmd, t);
        }
    ...
    __ejecutandoComando(cmd, t){

        console.log(`${t} @shell: ${cmd.get("comando")} ${cmd.get("args").join(" ")}`);
